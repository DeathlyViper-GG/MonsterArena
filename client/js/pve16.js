  ...
  function draw(dt) {
   ...
   // ---------------------------
    ...
    if (online) {
      ...
      const prev = _reconPrev;
      const next = [];

      const snapBullets =
        (snap && Array.isArray(snap.bullets))
          ? snap.bullets
          : [];
      ...
      for (const sb of snapBullets) {
        ...
        // ✅ SNAP SERVER BULLET TO LOCAL POSITION (NO PULLBACK)
        if (best !== -1 && bestD < 900) {
          fx = prev[best].x;
          fy = prev[best].y;
      ...
      // ✅ KEEP LOCAL BULLETS ONLY IF NOT YET MATCHED
      for (const lb of ents.bullets) {
